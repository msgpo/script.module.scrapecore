...
DB_TYPE = 'sqlite'
DB_FILE = kodi.vfs.join(kodi.get_addon('script.module.scrapecore').getAddonInfo('profile').decode('utf-8'), 'core.db')

	
class DBI(SQLiteDatabase):
	def _initialize(self):
		self.connect()
		schema_file = kodi.vfs.join(kodi.get_addon('script.module.scrapecore').getAddonInfo('path').decode('utf-8'), 'resources/database/schema.%s.sql' % self.db_type)	
